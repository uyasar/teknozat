  ...
  return (
    <main className="pt-24 pb-20 min-h-screen">
      <div className="section">
        <div className="mb-8">
          <h1 className="font-display font-bold text-3xl sm:text-4xl mb-1">Kurslar</h1>
          <p className="text-white/40 text-sm">{COURSES.length} kurs mevcut</p>
        </div>
            ...
            <div className="flex gap-1.5 flex-wrap">
              {LEVELS.map(l => (
                <button
                  ...
                  className={clsx(
                    'px-4 rounded-xl text-xs font-semibold transition-colors',
                    'min-h-[44px]',
                    levelFilter === l
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {filtered.map(course => (
            <Link key={course.id} to={`/dersler/${course.slug}`} className="card-hover group block">
              <div className="w-full aspect-video rounded-xl bg-gradient-to-br from-white/5 to-white/2
                border border-white/5 mb-4 flex items-center justify-center text-5xl relative overflow-hidden">
              ...
              <div className="flex items-start justify-between gap-2 mb-1.5">
                <h3 className="font-semibold text-sm group-hover:text-gold transition-colors leading-snug">
                  {course.title}
                </h3>
                <span className={clsx(
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20 text-white/30">
            ...
            <p className="text-sm">Aramanızla eşleşen kurs bulunamadı.</p>
          </div>
        )}
      </div>
    </main>
