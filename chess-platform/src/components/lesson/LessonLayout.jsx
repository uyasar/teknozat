...
export default function LessonLayout({ lesson }) {
  ...
  const handleDrop = useCallback((from, to) => {
    ...
    if (activeTab === 'puzzle' && currentPuzzle) {
      ...
      if (!ok) {
        setPuzzleStatus('wrong'); setAttempts(a => a + 1)
        setTimeout(() => setPuzzleStatus('idle'), 1200)
        return null
      }
      ...
      setTimeout(() => {
        try {
          ...
          if (r) {
            setPuzzleFen(g2.fen()); setPuzzleMvIdx(next + 1)
    ...
    activeTab, currentPuzzle, puzzleFen, puzzleMvIdx,
    aiMode, lesson?.pgn, currentIndex, makeMove,
  ])
  ...
  const handleSquareClick = useCallback((sq) => {
    ...
    if (selectedSq && selectedSq !== sq) {
      if (piece?.color === game.turn()) {
        setSelectedSq(sq); setLegalMoves(getLegalMovesFrom(sq)); return
      }
    ...
    if (piece?.color === game.turn()) {
      setSelectedSq(sq); setLegalMoves(getLegalMovesFrom(sq))
    } else {
      setSelectedSq(null); setLegalMoves([])
    }
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-5 w-full">
      ...
      {/* ══ Board column ══════════════════════════════════════════ */}
      <div ref={boardColRef} className="flex flex-col items-center gap-3 w-full xl:w-auto">
            ...
            {['auto', 'manual'].map(m => (
              <button
                ...
                className={clsx(
                  ...
                  aiMode === m ? 'bg-gold text-bg-base shadow-sm' : 'text-white/40 hover:text-white'
                )}
              >
        ...
        {/* Deviation alert */}
        {deviationInfo && (
          <div className="w-full rounded-xl bg-amber-500/10 border border-amber-500/25
            px-4 py-3 text-xs text-amber-200 leading-relaxed">
        ...
        <div className="flex items-center border-b border-white/5">
          {TABS.filter(t => t.id !== 'puzzle' || hasPuzzles).map(({ id, label, icon: Icon }) => (
            ...
            >
              ...
              {id === 'puzzle' && (
                <span className="ml-0.5 rounded-full bg-white/10 px-1.5 text-[9px]">
