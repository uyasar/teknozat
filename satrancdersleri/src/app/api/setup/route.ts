  ...
  try {
    ...
    const beginnerCourse = await prisma.course.create({
      data: {
        ...
        description: 'Satrancın temel kurallarını, taş hareketlerini ve temel taktikleri öğrenin.',
        type: 'ADULT',
        level: 'BEGINNER',
        published: true,
        order: 1,
    ...
    const lesson1 = await prisma.lesson.create({
      data: {
        ...
        description: 'Satrançtaki tüm taşların nasıl hareket ettiğini öğrenin.',
        order: 1,
        published: true,
    ...
    const lesson2 = await prisma.lesson.create({
      data: {
        ...
        description: 'Kale matı ve vezir matı gibi temel mat pozisyonlarını öğrenin.',
        order: 2,
        published: true,
        courseId: beginnerCourse.id,
      },
    ...
    const openingLesson = await prisma.lesson.create({
      data: {
        ...
        description: 'Klasik bir açılış olan İtalyan Açılışını öğrenin.',
        isOpening: true,
        order: 3,
        published: true,
        courseId: beginnerCourse.id,
    ...
    await prisma.lesson.create({
      data: {
        title: 'Çatal Taktiği',
        slug: 'catal-taktigi',
        description: 'Tek hamlede iki taşı aynı anda tehdit etme taktiği.',
        order: 1,
        published: true,
    ...
    await prisma.puzzle.create({
      data: {
        ...
        solution: ['Ng5'],
        hint: 'At f7 karesini hem şahı hem de veziri tehdit edebilir.',
        difficulty: 2,
        order: 1,
        lessonId: lesson1.id,
      },
    ...
    await prisma.famousGame.create({
      data: {
        ...
        year: 1851,
        result: '1-0',
        description: '"Ölümsüz Oyun" — satranç tarihinin en güzel oyunlarından biri.',
    ...
    return NextResponse.json(
      { error: err?.message ?? 'Bilinmeyen hata', code: err?.code },
      { status: 500 }
    )
  }
}
